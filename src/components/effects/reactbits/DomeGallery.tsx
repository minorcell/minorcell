...
const normalizeImageInput = (image: ImageInput) => {
  if (typeof image === 'string') {
    return { src: image.trim(), alt: '', href: '' }
  }
  return {
    src: (image.src || '').trim(),
    alt: image.alt || '',
...
function buildItems(pool: ImageInput[], seg: number): ItemDef[] {
  ...
  const fallbackImages = DEFAULT_IMAGES.map(normalizeImageInput).filter(
    (image) => image.src.length > 0,
  )

  const effectiveImages =
    normalizedImages.length > 0 ? normalizedImages : fallbackImages
  ...
  const usedImages = Array.from(
    { length: totalSlots },
    (_, i) => effectiveImages[i % effectiveImages.length],
  )
...
export default function DomeGallery({
  ...
  maxRadius = Infinity,
  padFactor = 0.25,
  overlayBlurColor = 'color-mix(in oklab, var(--background) 92%, var(--foreground) 8%)',
  maxVerticalRotationDeg = DEFAULTS.maxVerticalRotationDeg,
  dragSensitivity = DEFAULTS.dragSensitivity,
  enlargeTransitionMs = DEFAULTS.enlargeTransitionMs,
  segments = DEFAULTS.segments,
  dragDampening = 2,
  openedImageWidth = 'min(82vw, 720px)',
  openedImageHeight = 'calc(min(82vw, 720px) * 9 / 16)',
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (rawHref) {
      ...
      link.href = rawHref
      link.style.cssText = 'display:block; width:100%; height:100%; opacity:1;'
      link.setAttribute(
        'aria-label',
        rawAlt ? `Read ${rawAlt}` : 'Read article',
      )
  ...
  const cssStyles = `
    ...
    .item__image {
      ...
      left: 6px;
      width: calc(100% - 12px);
      height: calc((100% - 12px) * 9 / 16);
      top: calc(50% - (((100% - 12px) * 9 / 16) / 2));
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              ref={frameRef}
              className="viewer-frame h-full aspect-[16/9] flex"
              style={{
