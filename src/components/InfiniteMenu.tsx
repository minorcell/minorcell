...
class DiscGeometry extends Geometry {
  constructor(width = 1.6, height = 0.9) {
    ...
    this.vertices[0].uv = vec2.fromValues(0, 1)
    this.vertices[1].uv = vec2.fromValues(1, 1)
    this.vertices[2].uv = vec2.fromValues(1, 0)
    this.vertices[3].uv = vec2.fromValues(0, 0)
...
function createAndSetupTexture(
  ...
  // Seed with a visible pixel so the mesh does not render fully transparent
  // while the atlas is still loading.
  gl.texImage2D(
    gl.TEXTURE_2D,
    0,
    gl.RGBA,
    ...
    gl.RGBA,
    gl.UNSIGNED_BYTE,
    new Uint8Array([210, 210, 210, 255]),
  )
  return texture
}
...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discGeo = new DiscGeometry(1.6, 0.9)
  ...
  private initTexture(): void {
    ...
    Promise.all(this.items.map((item) => loadImage(item.image))).then(
      (images) => {
        ...
        images.forEach((img, i) => {
          ...
          if (img) {
            drawCover(img, x, y, cellWidth, cellHeight)
            return
          }
          ...
          const title = this.items[i]?.title?.trim() || '#'
          ctx.fillText(
        ...
        if (failed > 0) {
          console.warn(
            `[InfiniteMenu] ${failed} image(s) failed to load for atlas.`,
          )
        }
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  const titleText = truncateText(activeItem?.title || '', 28)
  const descriptionText = truncateText(activeItem?.description || '', 88)
    ...
    <div className="relative w-full h-full bg-background">
      ...
      {activeItem && (
        <>
          ...
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[100ms] translate-x-8'
              : 'opacity-100 pointer-events-auto duration-[500ms] translate-x-0'
          }
        `}
          >
            <h2 className="text-sm sm:text-base md:text-lg font-semibold leading-snug text-foreground">
              {titleText}
            </h2>
            <p className="mt-2 text-[11px] sm:text-xs md:text-sm leading-relaxed text-muted-foreground">
              {descriptionText}
            </p>
          </div>
