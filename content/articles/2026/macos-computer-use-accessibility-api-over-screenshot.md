...
keywords: [macOS, Computer Use, Accessibility API, AI操控, AppleScript]
order: 48
---
...
去年公司黑客松我们做了两个方案：Playwright 操控浏览器、VNC 沙箱截图识别做键鼠操作。但这两个都不算真正的 Computer Use——Playwright 依赖结构化 API，VNC 慢、不稳定且依赖隔离环境。

最近在做 macOS Agent 应用时，我开始认真想：在 macOS 上这件事到底应该怎么做？

## 两条路线：视觉 vs 结构化

社区方案大体分两类。

**截图视觉识别**（如 Anthropic Computer Use、UI-TARS）：截全屏图发给多模态模型，输出坐标操作。直觉但不稳定——复杂界面和深色模式下容易出错，每步都要大图导致 token 开销大。更根本的问题是：GUI 是给人看的，不是给 AI 用的。

**结构化控制**（类似 Playwright 读 DOM）：macOS 原生提供 Accessibility API（AXUIElement），可以读取几乎任何应用的 UI 层级树、元素属性和可执行动作。这套能力是为辅助障碍人群设计的，但恰好为 AI 提供了结构化、可读写的界面抽象。

我让 Codex 基于 AXSwift 写了个操作计算器的 demo，跑通后信心大增——不需要截图找"数字 7 在哪里"，直接就能拿到每个按钮的 role、title、enabled、frame 和 action。

这就是结构化控制的核心优势：**对 AI 来说最有价值的不是像素，而是语义。** AX 给出的确定性信息远比截图里的像素推断可靠，而且不依赖分辨率、主题或窗口遮挡。
...
从去年黑客松到现在，我对这个问题的理解经历了几次转变：最初觉得"截图 + 多模态"最自然，后来意识到 AI 不需要通过眼睛理解界面——它可以直接读取界面背后的结构化描述。**Computer Use 的本质不是让 AI 模拟人类的 GUI 操作，而是让 AI 通过比 GUI 更底层、更结构化的系统接口来控制计算机。** macOS 的无障碍能力恰好为此提供了丰富的系统级支持——AI Agent 在某种意义上，正是一个"非视觉用户"。
