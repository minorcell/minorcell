...
打开源码，看看模块如何划分、进程如何拆分、接口如何定义，当然很重要。但如果学习止步于此，我们得到的往往只是 What，也就是“术”：VS Code 使用了哪些模式，某类代码放在哪一层，模块之间怎样通信。

这些“术”，都是一个团队面对特定问题、在特定技术条件下给出的答案。它们可以帮助我们读懂 VS Code，却未必能直接解决下一个系统的问题。换一个业务场景、团队规模或技术环境，照搬同样的模式，很可能就不再适用。

我们更应该学的是架构的“道”：**面对一个还没有标准答案的问题，架构师为什么这样判断？当时有哪些约束，哪些技术已经成熟，哪些还没有？他们试过什么，又为什么放弃？最后的选择解决了什么，同时留下了什么代价？**

一个可行的办法，是回到过去，沿着系统的演进过程重新走一遍。也不应该站在今天倒推一个看似完美的解释，而是回到每个决策发生的时刻，看看当时的架构师掌握了哪些信息，又是怎样做取舍的。

所以，这篇文章我想先把时间拉回去：从 Monaco 的浏览器实验开始，看 VS Code 怎样一步步走向桌面、扩展生态、远程开发和 Web，最后又因为 Electron 的安全变化重构进程边界。
