...
keywords: [Go语言, Agent架构, AI SDK, 系统设计, mini-opencode]
order: 46
---
...
这个拆法表面上看很普通，但一个好的架构拆分本来就不该追求”聪明”，而该追求归属清楚、演进自然。

`config` 负责配置加载、系统提示词、工作区解析；`provider` 负责统一模型接口和多厂商适配；`core` 负责 agent loop、session、turn 执行；`tools` 负责工具注册、执行和运行时拦截；`tui` 负责终端交互；`cmd` 负责装配。

这套结构的好处是，你一旦开始写代码就知道新能力该长在哪、变化该落在哪层、问题该先看哪块。很多项目不是死在”做不出来”，而是死在”第二轮开始就不知道该怎么继续长”。

## 好的模块拆分是为了让每一层只处理自己该处理的问题

很多人一开始拆模块，容易变成”按文件类型拆”或”按功能名拆”，最后得到的是一组名字看起来合理、但职责边界非常模糊的目录。

mini-opencode 这次我特别在意一件事：**每一层到底在回答什么问题？**

比如 `provider` 回答的是”模型如何被统一接入”，`core` 回答的是”一轮 Agent 执行怎么闭环”，`tools` 回答的是”系统如何暴露和执行能力”，`tui` 回答的是”用户如何感知系统正在干什么”。

一旦这么想，模块边界就会非常清楚。而且这种拆法让你能同时照顾产品视角和工程视角——你怎么定义产品里的”能力单元”，最后几乎一定会反映在模块边界和接口设计上。
