...
type: article
title: '论 SVG 的潜力'
description: '从博客配图的演变说起：手画、AI 生图、再到 AI 生成 SVG。位图生成贵在图像 token，SVG 赢在只是文本；便宜只是开始，可编辑、可缩放、可动画，还能把算法演示做成动图——SVG 的潜力在 AI 时代才刚刚被释放。'
date: 2026-08-14
updated: 2026-08-15
tags: [SVG, AI, 图像生成, 博客, 效率]
keywords: [SVG, 矢量图, AI 生图, 博客配图, 图像生成, Nano Banana, 文生图, GIF, 算法动图]
order: 62
---
...
上面说的“活的”，是 SVG 自己的动画能力。最近写[《Lua 编程入门》](/articles/2026/lua-programming-getting-started)教程时，我又发现了另一种“动”法，这个发现更实在。

写 Dijkstra 最短路那节的时候，我想给算法配一个演示动画：节点逐个被“固定”、边逐条变绿、距离表逐格更新。这种图过去很贵——要么用 AE 逐帧做，要么录屏加后期，要么手写 Canvas 动画。但转念一想：既然 SVG 是代码，动画为什么不能也是代码？

**动画的本质是状态序列。** 而算法本来就是状态的变化。Dijkstra 的每一轮迭代，就是一个明确的快照：哪些点已固定、哪些边在检查、距离表是什么值。把这些状态写成数据，一个模板渲染出所有帧：

```python
# 每一帧都是一个状态描述，模板把它渲染成 SVG
{"title": "第 2 轮：固定 C，B 从 4 变 3",
 "fixed": ["A", "C"],                    # 已固定的点
 "hot_edges": [("B","C"), ("C","D")],    # 本轮检查的边
 "dist": {"B": 3, "D": 10, "E": 12}}     # 距离表
```
...
当然，不是所有内容都值得动。状态随时间变化的东西——算法迭代、状态机转换、协议握手、数据在管道里的流向——动图能把“时间”这个维度直接给出来；而结构关系、概念对比这类并置信息，静态图反而更清楚。

还有个小细节：为什么保存成 GIF 而不是直接用 SVG 动画？因为 GIF 在 `<img>` 里处处能播，博客、掘金、各种平台通吃；SMIL 动画的兼容性至今参差不齐。**SVG 是设计源文件，GIF 是发行格式**——和“SVG 画完导出 PNG 发布”是同一个思路的延续。
