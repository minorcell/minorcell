...
date: 2026-02-23
updated: 2026-05-10
keywords: [Memo Code, 提示词架构, 系统提示词, 上下文组装, Agent开发]
order: 44
---
...
核心思路：**把提示词当作动态装配的产品，而不是静态维护的文档**。分层解耦让每层独立变更，格式适配（Markdown / JSON / XML 各司其职），工具描述从运行时注册表动态生成，关键位置标注来源路径方便追溯，Skills 自动兼容主流 Agent 配置目录无需迁移。每个来源按需加载、按优先级拼接，最终交付给模型的是当前上下文的最优组合。
