...
> **注意**：这个实验展示的是链式法则的"分配逻辑"——如何把总变化追溯到每个因素的贡献。在真正的神经网络中，反向传播传递的是"误差对参数的梯度"，而且每经过一层激活函数，梯度还要乘以该激活函数的导数。不过核心直觉不变：误差信号沿着计算链往回传，每一步拆解出该步的"责任"。
...
```text
问题：错误如何变成学习信号？
方法：反向传播——将误差从输出层沿网络层层回传，利用链式法则为每个参数分配调整量。这一方法由沃伯斯（1974）首次提出但被忽视，1986 年由鲁梅尔哈特、辛顿和威廉姆斯独立重新发现并在《自然》发表实验验证。
