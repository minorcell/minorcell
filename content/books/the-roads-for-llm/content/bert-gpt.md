...
这一章把两种架构的设计逻辑和各自的选择讲清楚——以及，为什么今天所有的大语言模型都站在 GPT 的线路上。
...
到 2023 年，几乎所有的前沿大模型都是 Decoder-only Transformer——GPT-4、Claude、Gemini、Llama、Qwen、DeepSeek——无一例外。
...
为什么 GPT 胜出：续写是通用接口；Decode-only 的 Scaling 边际收益更优越；架构简洁（一阶段部署）。
今天：所有前沿大语言模型都是 Decoder-only Transformer，但 BERT 的两阶段训练模式（预训练→微调）遗留为今天模型训练的核心流程。
```
...
BERT 之后的两年，几乎所有 NLP 论文都以微调 BERT 为起点。但 GPT-2 和 GPT-3 逐步证明了——让 Decoder-only 架构做更大的预训练，在足够多的参数和数据的加持下，可以连续展现令人惊讶的通用能力。到 2023 年，BERT 路线的后继者相对沉寂了——Encoder-only 模型仍然在搜索、分类和嵌入任务中广泛使用，但前沿通用语言模型的竞争已经完全被 Decoder-only 取代。
